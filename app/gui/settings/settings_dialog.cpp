...
QString tz_name(const QTimeZone &tz) { return QString::fromLatin1(tz.id()); }

constexpr const char *const LAST_EXPORTED_FILE_KEY =
    "settings_dlg/last_exported";
constexpr const char *const LAST_IMPORTED_FILE_KEY =
    "settings_dlg/last_imported";

} // namespace

SettingsDialog::SettingsDialog(ClockApplication *app, size_t idx,
                               QWidget *parent)
    : QDialog(parent), ui(new Ui::SettingsDialog), app(app), _curr_idx(idx) {
  ui->setupUi(this);
...
void SettingsDialog::on_import_btn_clicked() {
  ...
  auto last_path =
      state.value(LAST_IMPORTED_FILE_KEY, QDir::homePath()).toString();
  // *INDENT-OFF*
  auto filename = QFileDialog::getOpenFileName(
      this, tr("Import settings from ..."), last_path,
...
void SettingsDialog::on_export_btn_clicked() {
  ...
  auto filename = QFileDialog::getSaveFileName(
      this, tr("Export settings to ..."), last_path,
...
void SettingsDialog::on_lang_tr_btn_clicked() {
  QDesktopServices::openUrl(
      QUrl("https://github.com/Kolcha/DigitalClock5/wiki/Translate"));
}
...
void SettingsDialog::on_tray_icon_s_action_enabled_clicked(bool checked) {
  auto act = checked ? ui->tray_icon_s_action_box->currentData()
                           .value<opt::TrayIconAction>()
                     : opt::NoAction;
  app->config()->global()->setTrayIconSingleClickAction(act);
  emit globalOptionChanged(opt::TrayIconSingleClickAction,
                           QVariant::fromValue(act));
}

void SettingsDialog::on_tray_icon_s_action_box_activated(int index) {
  auto act =
      ui->tray_icon_s_action_box->currentData().value<opt::TrayIconAction>();
  app->config()->global()->setTrayIconSingleClickAction(act);
  emit globalOptionChanged(opt::TrayIconSingleClickAction,
                           QVariant::fromValue(act));
}
...
void SettingsDialog::on_tray_icon_d_action_enabled_clicked(bool checked) {
  auto act = checked ? ui->tray_icon_d_action_box->currentData()
                           .value<opt::TrayIconAction>()
                     : opt::NoAction;
  app->config()->global()->setTrayIconDoubleClickAction(act);
  emit globalOptionChanged(opt::TrayIconDoubleClickAction,
                           QVariant::fromValue(act));
}

void SettingsDialog::on_tray_icon_d_action_box_activated(int index) {
  auto act =
      ui->tray_icon_d_action_box->currentData().value<opt::TrayIconAction>();
  app->config()->global()->setTrayIconDoubleClickAction(act);
  emit globalOptionChanged(opt::TrayIconDoubleClickAction,
                           QVariant::fromValue(act));
}
...
void SettingsDialog::on_format_help_btn_clicked() {
  QDesktopServices::openUrl(QUrl("https://github.com/Kolcha/DigitalClock5/wiki/"
...
void SettingsDialog::on_layout_cfg_help_btn_clicked() {
  QDesktopServices::openUrl(
      QUrl("https://github.com/Kolcha/DigitalClock5/wiki/Multiple-lines"));
}
...
void SettingsDialog::on_multi_timezone_help_btn_clicked() {
  QDesktopServices::openUrl(
      QUrl("https://github.com/Kolcha/DigitalClock5/wiki/Multiple-timezones"));
}
...
void SettingsDialog::on_select_colorization_color_clicked() {
  InstanceConfig &acfg = *app->config()->instance(_curr_idx);
  auto color =
      QColorDialog::getColor(acfg.getColorizationColor(), this, QString(),
...
void SettingsDialog::on_texture_group_clicked(bool checked) {
  auto tx_tt = checked
                   ? ui->tx_options_box->currentData().value<tx::TextureType>()
                   : tx::None;
...
void SettingsDialog::on_tx_options_box_currentIndexChanged(int index) {
  ...
  case tx::None:
    ...
    break;
  case tx::Color:
    connect(ui->tx_btn, &QToolButton::clicked, this,
            &SettingsDialog::tx_select_color);
    ...
    connect(ui->tx_option, &QCheckBox::toggled, ui->tx_btn,
            &QWidget::setDisabled);
    connect(ui->tx_option, &QCheckBox::clicked, this,
            &SettingsDialog::tx_use_system_color);
    break;
  case tx::Gradient:
    connect(ui->tx_btn, &QToolButton::clicked, this,
            &SettingsDialog::tx_select_gradient);
    ui->tx_option->hide();
    ui->tx_btn->setEnabled(true);
    break;
  case tx::Pattern:
    connect(ui->tx_btn, &QToolButton::clicked, this,
            &SettingsDialog::tx_select_pattern);
    ui->tx_option->setText(tr("stretch instead of tile"));
    ui->tx_option->show();
    ui->tx_option->setChecked(!acfg.getTexturePatternTile());
    ui->tx_btn->setEnabled(true);
    connect(ui->tx_option, &QCheckBox::clicked, this,
            &SettingsDialog::tx_pattern_stretch);
    break;
  }
}
...
void SettingsDialog::tx_select_color() {
  ...
  auto color = QColorDialog::getColor(acfg.getTextureColor(), this, QString(),
                                      QColorDialog::ShowAlphaChannel);
  if (!color.isValid())
    return;
...
void SettingsDialog::tx_select_gradient() {
  ...
  // *INDENT-OFF*
  auto gradient =
      GradientDialog::getGradient(&ok, acfg.getTextureGradient(), this);
  // *INDENT-ON*
  if (!ok)
    return;
  gradient.setCoordinateMode(QGradient::ObjectMode);
  acfg.setTextureGradient(gradient);
  emit instanceOptionChanged(_curr_idx, opt::TextureGradient,
...
void SettingsDialog::tx_select_pattern() {
  ...
  // *INDENT-OFF*
  auto file =
      QFileDialog::getOpenFileName(this, QString(), acfg.getTexturePattern(),
...
void SettingsDialog::on_background_group_clicked(bool checked) {
  auto bg_tt = checked
                   ? ui->bg_options_box->currentData().value<tx::TextureType>()
                   : tx::None;
...
void SettingsDialog::on_bg_options_box_currentIndexChanged(int index) {
  ...
  case tx::None:
    ...
    break;
  case tx::Color:
    connect(ui->bg_btn, &QToolButton::clicked, this,
            &SettingsDialog::bg_select_color);
    ...
    connect(ui->bg_option, &QCheckBox::toggled, ui->bg_btn,
            &QWidget::setDisabled);
    connect(ui->bg_option, &QCheckBox::clicked, this,
            &SettingsDialog::bg_use_system_color);
    break;
  case tx::Gradient:
    connect(ui->bg_btn, &QToolButton::clicked, this,
            &SettingsDialog::bg_select_gradient);
    ui->bg_option->hide();
    ui->bg_btn->setEnabled(true);
    break;
  case tx::Pattern:
    connect(ui->bg_btn, &QToolButton::clicked, this,
            &SettingsDialog::bg_select_pattern);
    ui->bg_option->setText(tr("stretch instead of tile"));
    ui->bg_option->show();
    ui->bg_option->setChecked(!acfg.getBackgroundPatternTile());
    ui->bg_btn->setEnabled(true);
    connect(ui->bg_option, &QCheckBox::clicked, this,
            &SettingsDialog::bg_pattern_stretch);
    break;
  }
}
...
void SettingsDialog::bg_select_color() {
  auto &acfg = *app->config()->instance(_curr_idx);
  auto color =
      QColorDialog::getColor(acfg.getBackgroundColor(), this, QString(),
                             QColorDialog::ShowAlphaChannel);
  if (!color.isValid())
    return;
...
void SettingsDialog::bg_select_gradient() {
  ...
  // *INDENT-OFF*
  auto gradient =
      GradientDialog::getGradient(&ok, acfg.getBackgroundGradient(), this);
  // *INDENT-ON*
  if (!ok)
    return;
  gradient.setCoordinateMode(QGradient::ObjectMode);
  acfg.setBackgroundGradient(gradient);
  emit instanceOptionChanged(_curr_idx, opt::BackgroundGradient,
                             QVariant::fromValue(gradient));
}
...
void SettingsDialog::bg_select_pattern() {
  ...
  // *INDENT-OFF*
  auto file =
      QFileDialog::getOpenFileName(this, QString(), acfg.getBackgroundPattern(),
                                   tr("Images (*.png *.bmp *.jpg)"));
  // *INDENT-ON*
  if (file.isEmpty())
    return;
...
void SettingsDialog::onCharMarginsChanged() {
  QMarginsF m(ui->m_char_l->value(), ui->m_char_t->value(),
              ui->m_char_r->value(), ui->m_char_b->value());
  app->config()->instance(_curr_idx)->setCharMargins(m);
  emit instanceOptionChanged(_curr_idx, opt::CharMargins,
                             QVariant::fromValue(m));
}

void SettingsDialog::onTextMarginsChanged() {
  QMarginsF m(ui->m_text_l->value(), ui->m_text_t->value(),
              ui->m_text_r->value(), ui->m_text_b->value());
  app->config()->instance(_curr_idx)->setWidgetMargins(m);
  emit instanceOptionChanged(_curr_idx, opt::WidgetMargins,
                             QVariant::fromValue(m));
}
...
void SettingsDialog::initAppGlobalTab() {
  ...
  ui->tray_icon_s_action_enabled->setChecked(tray_icon_s_action !=
                                             opt::NoAction);
  ui->tray_icon_d_action_enabled->setChecked(tray_icon_d_action !=
                                             opt::NoAction);
  if (tray_icon_s_action == opt::NoAction)
    tray_icon_s_action = opt::ShowHideClock;
  if (tray_icon_d_action == opt::NoAction)
    tray_icon_d_action = opt::OpenSettings;
  setIndexByValue(ui->tray_icon_s_action_box,
                  QVariant::fromValue(tray_icon_s_action));
  setIndexByValue(ui->tray_icon_d_action_box,
                  QVariant::fromValue(tray_icon_d_action));
#else
...
void SettingsDialog::initGeneralTab(size_t idx) {
  ...
  ui->format_edit->setCurrentText(time_format);
  // in case of the same format as default in the UI design file signal is not
  // called
  ...
  if (!ui->use_custom_format->isChecked()) {
    ...
    ui->leading_zero->setChecked(
        time_format.contains("hh", Qt::CaseInsensitive));
  ...
  // but the same logic should be triggered on initialization (regardless of the
  // state)
  ...
  connect(ui->rb_12h, &QRadioButton::clicked, this,
          &SettingsDialog::updateTimeFormat);
  connect(ui->rb_24h, &QRadioButton::clicked, this,
          &SettingsDialog::updateTimeFormat);
  connect(ui->leading_zero, &QCheckBox::clicked, this,
          &SettingsDialog::updateTimeFormat);
  connect(ui->show_seconds, &QCheckBox::clicked, this,
          &SettingsDialog::updateTimeFormat);

  connect(ui->show_apm, &QCheckBox::clicked, this,
          &SettingsDialog::updateTimeFormat);
  connect(ui->add_space_before_apm, &QCheckBox::clicked, this,
          &SettingsDialog::updateTimeFormat);
  connect(ui->rb_ucase_apm, &QRadioButton::clicked, this,
          &SettingsDialog::updateTimeFormat);
  connect(ui->rb_lcase_apm, &QRadioButton::clicked, this,
          &SettingsDialog::updateTimeFormat);
...
void SettingsDialog::initAppearanceTab(size_t idx) {
  ...
  ui->skin_cbox->setCurrentIndex(
      -1); // if skin is available, next line will update the index
  ui->skin_cbox->setCurrentText(
      app->getSkinManager()->info(acfg.getSkin()).metadata["name"]);
  ...
  if (acfg.getTextureType() != tx::None)
    setIndexByValue(ui->tx_options_box,
                    QVariant::fromValue(acfg.getTextureType()));
  ui->tx_per_element_cb->setChecked(acfg.getTextureCustomization() ==
                                    LinesRenderer::PerChar);
  ...
  if (acfg.getBackgroundType() != tx::None)
    setIndexByValue(ui->bg_options_box,
                    QVariant::fromValue(acfg.getBackgroundType()));
  ui->bg_per_element_cb->setChecked(acfg.getBackgroundCustomization() ==
                                    LinesRenderer::PerChar);
  ...
  ui->anchor_left->setChecked(app->window(idx)->anchorPoint() ==
                              ClockWindow::AnchorLeft);
  ui->anchor_center->setChecked(app->window(idx)->anchorPoint() ==
                                ClockWindow::AnchorCenter);
  ui->anchor_right->setChecked(app->window(idx)->anchorPoint() ==
                               ClockWindow::AnchorRight);
...
void SettingsDialog::initMiscTab(size_t idx) {
  ...
  connect(ui->m_char_l, &QDoubleSpinBox::valueChanged, this,
          &SettingsDialog::onCharMarginsChanged);
  connect(ui->m_char_t, &QDoubleSpinBox::valueChanged, this,
          &SettingsDialog::onCharMarginsChanged);
  connect(ui->m_char_r, &QDoubleSpinBox::valueChanged, this,
          &SettingsDialog::onCharMarginsChanged);
  connect(ui->m_char_b, &QDoubleSpinBox::valueChanged, this,
          &SettingsDialog::onCharMarginsChanged);
  ...
  connect(ui->m_text_l, &QDoubleSpinBox::valueChanged, this,
          &SettingsDialog::onTextMarginsChanged);
  connect(ui->m_text_t, &QDoubleSpinBox::valueChanged, this,
          &SettingsDialog::onTextMarginsChanged);
  connect(ui->m_text_r, &QDoubleSpinBox::valueChanged, this,
          &SettingsDialog::onTextMarginsChanged);
  connect(ui->m_text_b, &QDoubleSpinBox::valueChanged, this,
          &SettingsDialog::onTextMarginsChanged);
...
void SettingsDialog::initPluginsTab() {
  ...
  auto icfg = app->config()->instance(_curr_idx);
  auto disabled = icfg->getDisabledPlugins();
  ...
  for (const PluginHandle &p : std::as_const(plugins)) {
    ...
    // checked = globally active AND not disabled for this window
    widget->SetChecked(p.isActive() && !disabled.contains(p.id()));
    // *INDENT-OFF*
    connect(widget, &PluginListItemWidget::StateChanged, this,
            [this, p](bool checked) { onPluginStateChanged(p, checked); });
    connect(widget, &PluginListItemWidget::ConfigureRequested,
            [this, ph = p]() mutable { ph.configure(this, _curr_idx); });
    connect(widget, &PluginListItemWidget::InfoDialogRequested, this,
            [this, p]() { showPluginInfoDialog(p); });
    // *INDENT-ON*
...
void SettingsDialog::onPluginStateChanged(PluginHandle ph, bool enabled) {
  auto icfg = app->config()->instance(_curr_idx);
  auto disabled = icfg->getDisabledPlugins();
  auto global_plugins = app->config()->global()->getPlugins();

  if (enabled) {
    // remove from disabled list for this window
    disabled.removeOne(ph.id());
    icfg->setDisabledPlugins(disabled);

    if (!ph.isActive()) {
      // globally inactive: activate globally
      // first, disable for all OTHER windows so it only starts for this one
      const auto act_insts = app->config()->global()->getActiveInstancesList();
      for (auto idx : act_insts) {
        if (idx != _curr_idx) {
          auto other_cfg = app->config()->instance(idx);
          auto other_disabled = other_cfg->getDisabledPlugins();
          if (!other_disabled.contains(ph.id())) {
            other_disabled.push_back(ph.id());
            other_cfg->setDisabledPlugins(other_disabled);
          }
        }
      }
      // now globally activate - respects per-instance disabled lists
      ph.setActive(true);
      global_plugins.push_back(ph.id());
      app->config()->global()->setPlugins(global_plugins);
    } else {
      // globally active: just enable for this instance
      ph.setInstanceEnabled(_curr_idx, true);
    }
  } else {
    // add to disabled list for this window
    if (!disabled.contains(ph.id()))
      disabled.push_back(ph.id());
    icfg->setDisabledPlugins(disabled);

    if (ph.isActive()) {
      // shutdown instance for this window
      ph.setInstanceEnabled(_curr_idx, false);

      // if disabled in ALL windows, globally deactivate
      bool disabledEverywhere = true;
      const auto act_insts = app->config()->global()->getActiveInstancesList();
      for (auto idx : act_insts) {
        auto other_disabled =
            app->config()->instance(idx)->getDisabledPlugins();
        if (!other_disabled.contains(ph.id())) {
      ...
      if (disabledEverywhere) {
        ph.setActive(false);
        global_plugins.removeOne(ph.id());
        app->config()->global()->setPlugins(global_plugins);
      }
    }
  }
...
void SettingsDialog::fillLanguagesList() {
  ...
  for (const auto &l : ll)
    ui->lang_list->addItem(sl.value(l).toString(), l);
  sl.endGroup();
}
...
void SettingsDialog::fillTrayIconActions(QComboBox *box) {
  ...
  box->addItem(tr("toggle \"stay on top\""),
               QVariant::fromValue(opt::ToggleStayOnTop));
}
...
void SettingsDialog::fillTimeZonesList() {
  ...
  for (const auto &tz_id : QTimeZone::availableTimeZoneIds()) {
    ...
    auto tooltip =
        QString("%1 (%2)").arg(tz.displayName(now, QTimeZone::LongName),
                               tz.displayName(now, QTimeZone::OffsetName));
    ui->time_zone_edit->setItemData(ui->time_zone_edit->count() - 1, tooltip,
                                    Qt::ToolTipRole);
  }
}
...
void SettingsDialog::fillSkinsList() {
  ...
  auto locale_cmp = [](QStringView lhs, QStringView rhs) {
    return QString::localeAwareCompare(lhs, rhs) < 0;
  };
  std::multimap<QString, QString, decltype(locale_cmp)> sorted_skins(
      locale_cmp);
