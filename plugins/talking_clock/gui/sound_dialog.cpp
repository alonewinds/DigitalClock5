  ...
  , ui(new Ui::SoundDialog)
{
  ...
  ui->defaults_box->addItem(tr("sound 1"), QUrl("qrc:/talking_clock/hour_signal.wav"));
  ui->defaults_box->addItem(tr("sound 2"), QUrl("qrc:/talking_clock/quarter_signal.wav"));
  ui->defaults_box->addItem(tr("sound 3"), QUrl("qrc:/talking_clock/big_ben_strikes_1.mp3"));
  ui->defaults_box->addItem(tr("sound 4"), QUrl("qrc:/talking_clock/quarter_hour_chime_beg.mp3"));
...
void SoundDialog::onPlayerStateChanged(QMediaPlayer::PlaybackState new_state)
{
  if (new_state == QMediaPlayer::PlayingState) {
    ...
    ui->preview_btn->setIcon(QIcon(":/talking_clock/stop.svg"));
  } else {
    disconnect(ui->preview_btn, &QToolButton::clicked, _player, &QMediaPlayer::stop);
    connect(ui->preview_btn, &QToolButton::clicked, _player, &QMediaPlayer::play);
    ui->preview_btn->setIcon(QIcon(":/talking_clock/play.svg"));
  }
}
